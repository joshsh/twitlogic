package net.fortytwo.twitlogic.server.rewriter;
    ...
    public CloseableIteration<? extends Statement, SailException> getStatements(Resource subj,
        ...
        for (int i = 0; i < contexts.length; i++) {
            if (contexts[i] instanceof URI) {
                contexts[i] = rewriters.getRewriter(RewritingSchema.PartOfSpeech.GRAPH, RewritingSchema.Action.TO_STORE).rewrite((URI) contexts[i]);
            }
        }
        ...
        public Statement next() throws SailException {
            ...
            if (null != context && context instanceof URI) {
                context = rewriters.getRewriter(
                        RewritingSchema.PartOfSpeech.GRAPH, RewritingSchema.Action.FROM_STORE)
                        .rewrite((URI) context);
            }
